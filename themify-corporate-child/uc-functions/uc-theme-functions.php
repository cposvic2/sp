...
$authority_templates = array(
	...
	'page-sponsorship-info.php' => 'edit_class',
);
...
add_action('wp_loaded', 'uc_start_session', 1);
...
function uc_post_submit_handler() {
	if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
		if(isset( $_POST['action']) ) {
			if ( $_POST['action'] == 'sponsorship' ) {
				// Nonce check
				if ( wp_verify_nonce( $_POST['sponsorship_noncename'], 'sponsorship') ) {
					$class_id = ( isset( $_POST['class_id'] ) ? intval( $_POST['class_id'] ) : null );
					$uname = ( isset( $_POST['uname'] ) ? sanitize_user( $_POST['uname'] ) : null );
					$upass = ( isset( $_POST['upass'] ) ? $_POST['upass'] : null );
					$name = ( isset( $_POST['name'] ) ? sanitize_text_field( $_POST['name'] ) : null ); 
					$email = ( isset( $_POST['email'] ) ? sanitize_email( $_POST['email'] ) : null );
					$twitter = ( isset( $_POST['twitter'] ) ? sanitize_text_field( $_POST['twitter'] ) : null );
					$stripe_token = ( isset( $_POST['stripe_token'] ) ? $_POST['stripe_token'] : null );

					if (!$class_id) {
						$GLOBALS['response'] = array(
							'status' => 'REQUEST_DENIED',
							'reason' => 'Your request was invalid. No class ID.'
						);
						return false;
					}
					...
					if (!$name ) {
						$GLOBALS['response'] = array(
							'status' => 'REQUEST_DENIED',
							'reason' => 'You did not submit a valid name. Please try again.'
						);
						return false;
					}
					...
					if (!$email ) {
						$GLOBALS['response'] = array(
							'status' => 'REQUEST_DENIED',
							'reason' => 'You did not submit a valid email address. Please try again.'
						);
						return false;
					}
					...
					if (!$stripe_token ) {
						$GLOBALS['response'] = array(
							'status' => 'REQUEST_DENIED',
							'reason' => 'Your credit card was not submitted. Please try again.'
						);
						return false;
					}

					$stripe_id = stripe_save_credit_card( $stripe_token, $email );
					if (!$stripe_id ) {
						$GLOBALS['response'] = array(
							'status' => 'REQUEST_DENIED',
							'reason' => 'Your credit card was not processed. Please try again.'
						);
						return false;
					}

					$price = (int)vp_option('uc_option.sponsor_price');
					$stripe_charge = stripe_charge_customer( $stripe_id, $price );
					if (!$stripe_charge ) {
						$GLOBALS['response'] = array(
							'status' => 'REQUEST_DENIED',
							'reason' => 'There was an error processing your credit card. Please try again.'
						);
						return false;
					}
					...
					if (!!$upass) {
						$new_user_id = create_sponsor_user( $email, $upass, $name, $email, $twitter, $stripe_id );
					}

					$new_sponsorship = array (
						'post_type' => 'uc_sponsorship',
						'post_title' => $class_name.' by '.$name,
						'post_status' => 'publish',
					);

					if (!!$new_user_id) {
						$new_sponsorship['post_author'] = $new_user_id;
					}
					
					$new_sponsorship_id = wp_insert_post( $new_sponsorship );

					global $uc_sponsorship_fields;
					update_post_meta( $new_sponsorship_id, 'uc_sponsorship_fields', $uc_sponsorship_fields );
					update_post_meta( $new_sponsorship_id, 'fufilled', 0 );

					if (!$new_user_id) {
						update_post_meta( $new_sponsorship_id, 'sponsor_name', $name );
						update_post_meta( $new_sponsorship_id, 'sponsor_twitter', $twitter );
						update_post_meta( $new_sponsorship_id, 'sponsor_email', $email );
						update_post_meta( $new_sponsorship_id, 'sponsor_stripe', $stripe_id );
					} else {
						update_post_meta( $new_sponsorship_id, 'user_id', $new_user_id );
					}

					update_post_meta( $new_sponsorship_id, 'class', $class_id );
					update_post_meta( $new_sponsorship_id, 'stripe_charge', $stripe_charge );

					send_sponsorship_email( $new_sponsorship_id );

					// Redirect to this page.
					header("Location: " . home_url() . '/sponsorship-confirmation/' );
					exit();
			...
			} elseif ( $_POST['action'] == 'activeClass' ) {
				// Nonce check
				...
				if ( wp_verify_nonce( $_POST['update_user_noncename'], 'update_user') ) {
					...
					$birthday = ( isset( $_POST['birthdate'] ) && strtotime( $_POST['birthdate'] ) ? date("m-d-Y", strtotime( sanitize_text_field( $_POST['birthdate'] ))) : false ); 
					$gender = ( isset( $_POST['gender'] ) ? sanitize_text_field( $_POST['gender'] ) : false );
					$stem_proficient = ( isset( $_POST['stem_proficient'] ) ? intval( $_POST['stem_proficient'] ) : 0 );
					$college_science_major = ( isset( $_POST['college_science'] ) ? intval( $_POST['college_science'] ) : 0 );
					$ambassador = ( isset( $_POST['uamb'] ) ? intval( $_POST['uamb'] ) : 0 );
					$school_at_risk = ( isset( $_POST['school_at_risk'] ) ? intval( $_POST['school_at_risk'] ) : 0 );
					$stem_competitions = ( isset( $_POST['stem_competitions'] ) ? intval( $_POST['stem_competitions'] ) : 0 );
					$mailing_list = ( isset( $_POST['mailing_list'] ) ? intval( $_POST['mailing_list'] ) : 0 );
					...
					if ( $user_created ) {
						...
						$notification_emails = intval($_POST['notification_emails']);
						for ($i=1; $i <= $notification_emails; $i++) { 
							$notify_question = sanitize_email( $_POST['notify_question_'.$i] );
							if (!!$notify_question) {
								uc_email_admin_notify( $user_created, $notify_question );
							}
						}
				...
				if ( wp_verify_nonce( $_POST['update_school_noncename'], 'update_school') && current_user_can('edit_school') ) {
					...
					if ( $school_created ) {
						...
						if ( !current_user_can( 'edit_class' ) ) {
							add_teacher_to_mailing_list( $teacher_id );
...
function wp_redirect_visitors() {
	...
	if ( $page_slug == 'confirm' ) {
		...
		if ( isset( $_GET["confirmation_token"] ) ) {
			...
			$teacher_id = get_user_meta( $verified_user, 'teacher_id', 'true' );
			uc_should_schedule_email_missing_data( $teacher_id );
		}
...
function get_post_or_saved_postdata( $tag, $default = '', $post_id = null ) {
	if (isset($_POST[$tag]))
		return $_POST[$tag];
	...
	switch ($tag) {
		case 'biz_title':
			...
			break;
		default:
			$saved = get_post_meta($post_id, $tag, true);
			if ($saved !== false)
				return $saved;
			break;
	}
