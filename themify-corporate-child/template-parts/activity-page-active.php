...
<div class="activity-class-data">
	...
	<div class="activity-class-data-details">
		...
		$activity_text = str_replace ( '!days!' , $expiration_length, vp_option('uc_option.a_active_text') );
		...
		$max_teamwork_5 = vp_option('uc_option.max_teamwork_5');
		$score_ranges = get_score_ranges( get_the_id() );

		// Errors
		$uc_errors = array();
		$uc_errors['max_teamwork_5'] = vp_option('uc_option.max_teamwork_5');
		$uc_errors['a_teamwork_5'] = vp_option('uc_option.a_teamwork_5');
		$uc_errors['a_changed_teams'] = vp_option('uc_option.a_changed_teams');
		$uc_errors['a_missing_scores'] = vp_option('uc_option.a_missing_scores');

		echo '<script type="text/javascript">';
			foreach ( $uc_errors as $key => $uc_error ) {
				echo 'var ' . $key . '="' . $uc_error . '";';
			}
		//echo 'var score_ranges = '.json_encode($score_ranges).';';
		echo '</script>';
		// end Errors
		echo '<p><?php echo $activity_text ?></p>
			<p><strong>Signup Date: </strong>'.$signup_date->format('M j Y').'<br>
			<strong>Expiration Date:</strong> '.$expiration_date->format('M j Y').'</p>
			<div class="group-modify-container" data-ref="'.$active_class.'" data-activity="'.get_the_id().'">';
		display_groups( $active_class, get_the_id() );
		echo '<div class="alert-container"></div>';
		wp_nonce_field( 'update_groups', 'update_groups_noncename' );
		echo '
			<input type="submit" class="group-save button" value="Save Teams">
			<input type="submit" class="group-submit button" value="Submit Scores">
		</div>';
 		?>
	</div>
